...
if args.method == 'msp':
    ...
    raise ValueError(f'Unsupported method for discuss.py: {args.method}')
...
if args.dis_type == 'vis_energy':
    if not os.path.exists('results/vis_scores'):
        os.makedirs('results/vis_scores')
    if args.use_prop:
    ...
    filename = f'results/vis_scores/{args.dataset}-{name}.csv'
