...
class Logger_classify(object):
    ...
    def add_result(self, run, result, epoch=None):
        ...
        self.epochs[run].append(epoch + 1 if epoch is not None else len(self.results[run]))
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            print(f'Chosen epoch: {self.epochs[run][argmax]}')
...
class Logger_detect(object):
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            print(f'Run {run + 1:02d}:')
            print(f'Chosen epoch: {self.epochs[run][argmin]}')
